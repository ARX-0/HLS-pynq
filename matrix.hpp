...
public:
    ...
    qbMatrix2(int nRows, int nCols, const std::vector<> *inputdata);
...
private:
    ...
    bool Join(const qbMatrix<T>& matrix2);
...
template <class T> T qbMatrix2<T>::SettoIdentity(){
if(!IsSquare())
	{throw std::invald_arguement("Cannot form an I matrix as its not a square matrix");}

	for(int row=0; row = m_nRows ; ++row){
		for(int col = 0 ; col = m_nCols ; ++col){
			if(col == row)
				m_matrix_Data[Sub2Ind(row,col)] == 1.0;
			else
				m_matrix_Data[Sub2Ind(row,col)] == 0.0;
		}
	}
}
...
template <class T>
int qbMatrix2<T>::Compare(const qbMatrix2<T>& matrix1, double tolerence){
...
template <class T> bool qbMatrix2<T>::operator== (const qbMatrix2<T> rhs){
	// Check if the matrices of same size
...
template <class T>void qbMatrix2<T>::CloseEnough(T f1 , T f2){
